-- Migration: Enhance cancellation function to manage slot availability
-- Date: 2025-07-02

-- Update the cancellation function to also update slot availability
...
BEGIN
  ...
  -- Update booking status with explicit parameter reference
  ...
  -- Update slot availability - make the slot available again
  ...
  IF NOT is_late THEN
    ...
    IF time_slot_record.max_students > 1 THEN
      ...
      result := json_build_object(
        ...
        'slot_freed', true,
        'message', 'Lesson cancelled successfully. No make-up credit issued for group sessions as per policy. The time slot is now available for other students.'
      );
    ELSE
      ...
      IF current_month_credits < 2 THEN
        ...
        result := json_build_object(
          ...
          'slot_freed', true,
          'message', 'Lesson cancelled successfully. A make-up lesson credit has been added to your account. The time slot is now available for other students.'
        );
      ELSE
        ...
        result := json_build_object(
          ...
          'makeup_credit_created', false,
          'slot_freed', true,
          'message', 'Lesson cancelled successfully. No make-up credit issued as you have reached the monthly limit of 2 credits. The time slot is now available for other students.'
        );
      END IF;
    END IF;
  ELSE
    ...
    result := json_build_object(
      ...
      'makeup_credit_created', false,
      'slot_freed', true,
      'message', 'Lesson cancelled but forfeited due to late cancellation. The time slot is now available for other students.'
    );
  END IF;
