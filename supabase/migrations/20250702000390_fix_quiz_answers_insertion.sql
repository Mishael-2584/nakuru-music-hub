-- Fix quiz answers insertion in RPC function
-- Date: 2025-07-02
...
BEGIN
  ...
  LOOP
    ...
    IF FOUND THEN
      ...
      answer_points := COALESCE((answer_data->>'pointsEarned')::INTEGER, 0);
      is_correct_answer := COALESCE((answer_data->>'isCorrect')::BOOLEAN, false);
      ...
      INSERT INTO quiz_submission_answers (
        ...
        question_record.id,
        CASE 
          WHEN (answer_data->>'selectedAnswerId') IS NOT NULL AND (answer_data->>'selectedAnswerId') != 'null'
          THEN (answer_data->>'selectedAnswerId')::UUID
          ELSE NULL
        END,
        COALESCE(answer_data->'matchingPairs', '[]'::jsonb),
        is_correct_answer,
        answer_points
      );
