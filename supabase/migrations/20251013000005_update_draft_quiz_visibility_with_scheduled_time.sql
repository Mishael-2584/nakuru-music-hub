-- Update draft quiz visibility logic
-- Date: 2025-10-13
-- New requirements:
-- 1. Draft quiz WITH scheduled time: VISIBLE to students but shows "restricted until [time]"
-- 2. Draft quiz WITHOUT scheduled time: INVISIBLE to students
-- 3. Published quiz BEFORE scheduled time: VISIBLE to students but not accessible
-- 4. Published quiz AFTER scheduled time: VISIBLE and accessible to students
...
-- Add comments
COMMENT ON FUNCTION get_classroom_feed(UUID) IS 'Returns classroom feed with updated visibility: teachers see all posts, students see published quizzes and draft quizzes with scheduled times (shown as restricted), but NOT draft quizzes without scheduled times';
