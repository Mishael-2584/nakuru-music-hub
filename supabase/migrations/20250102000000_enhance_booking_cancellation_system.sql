...
DECLARE
  ...
  end_date DATE := COALESCE(NEW.end_date, NEW.start_date + INTERVAL '6 months');
  booking_date DATE;
  booking_id UUID;
BEGIN
  -- Generate bookings for the recurring pattern
  WHILE current_date <= end_date AND NEW.is_active = true LOOP
    -- Check if this day of week matches the pattern
    IF EXTRACT(DOW FROM current_date) = 
...
-- Create trigger for recurring booking generation
CREATE TRIGGER trigger_generate_recurring_bookings
  AFTER INSERT ON public.recurring_booking_patterns
  FOR EACH ROW
