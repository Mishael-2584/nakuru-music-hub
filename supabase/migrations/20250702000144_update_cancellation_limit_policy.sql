...
BEGIN
  ...
  SET 
    ...
    forfeited = current_cancellations >= 2
  WHERE id = booking_id_param;

  IF (can_receive_result->>'can_receive')::BOOLEAN THEN
    INSERT INTO public.makeup_credits (
      student_id, teacher_id, credit_type, reason, expires_at, created_at
    ) VALUES (
      student_id_val, booking_record.teacher_id, 'cancellation', COALESCE(cancellation_reason_param, 'Lesson cancelled'),
      get_end_of_month(CURRENT_DATE), NOW()
