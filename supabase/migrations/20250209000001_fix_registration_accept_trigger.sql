-- Fix: Allow registration status update to succeed when student already exists
-- The trigger create_student_from_registration fails if a student with the same email
-- already exists (e.g. from a previous approval attempt). Use ON CONFLICT to handle this.
