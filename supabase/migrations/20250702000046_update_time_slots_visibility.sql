-- Migration: Update time slots visibility to return only open (unfilled) slots
-- Date: 2025-07-02

-- Drop and recreate the function to adjust return type and logic
DROP FUNCTION IF EXISTS get_available_time_slots_for_student(UUID);
...
BEGIN
  ...
  LOOP
    ...
    -- Count existing bookings for this slot on the next date
    SELECT COUNT(*)::INTEGER INTO booked_count
    FROM public.bookings b
    WHERE b.time_slot_id = slot_record.id
      AND b.booking_date = next_date
      AND b.status = 'confirmed';

    -- Only return slots that still have capacity remaining
    IF booked_count < COALESCE(slot_record.max_students, 1) THEN
      RETURN QUERY SELECT 
        slot_record.id,
