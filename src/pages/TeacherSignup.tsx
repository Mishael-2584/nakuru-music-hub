  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const uploadFile = async (file: File, type: string) => {
        const fname = buildUploadFileName(email, type, file.name);
        const { data, error: upErr } = await supabase.storage
          .from("teacher-cvs")
          .upload(fname, file, { upsert: false, contentType: file.type || undefined });
        if (upErr) throw upErr;
        return data?.path || fname;
      };

      const cvFilePath = await uploadFile(cvFile, "cv");

      // Client-generated id — do not call .select("id") (that requires extra SELECT RLS)
      ...
      const saveDocument = async (file: File, type: string) => {
