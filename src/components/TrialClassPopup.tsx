        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
            {benefits.map((benefit, index) => (
              <div key={index} className="flex items-center gap-2 p-2 sm:p-3 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg border">
                <benefit.icon className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 flex-shrink-0" />
                <div className="min-w-0">
                  <h4 className="font-semibold text-xs sm:text-sm text-blue-900">{benefit.title}</h4>
                  <p className="text-xs text-blue-700 truncate">{benefit.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1 sm:gap-2">
              {subjects.map((subject, index) => (
                <div key={index} className="flex items-center gap-1 sm:gap-2 p-1 sm:p-2 bg-gray-50 rounded-lg">
                  <subject.icon className={`w-3 h-3 sm:w-4 sm:h-4 ${subject.color} flex-shrink-0`} />
                  <span className="text-xs sm:text-sm font-medium truncate">{subject.name}</span>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gradient-to-r from-green-50 to-blue-50 p-3 sm:p-4 rounded-lg border">
            <h3 className="font-semibold text-base sm:text-lg mb-2 text-green-800">What You'll Get:</h3>
          ...
          <div className="flex flex-col gap-2 sm:gap-3">
            <Button asChild className="w-full bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-white font-semibold py-2 sm:py-3">
              <Link to="/trial-classes" onClick={handleClose}>
                <Gift className="w-4 h-4 mr-2" />
                Book Free Trial Class
              </Link>
            </Button>
            <Button variant="outline" asChild className="w-full py-2 sm:py-3">
