...
export default function ManualInvoiceManager({ invoice, onUpdate }: ManualInvoiceManagerProps) {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          invoice_details: invoiceDetails, // Also save in invoice_details
          manual_amount_override: calculatedAmount !== invoice.amount_due ? calculatedAmount : null,
  ...
  const handleClearOverride = async () => {
    if (!confirm('Are you sure you want to clear the manual override and restore the original amount?')) {
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: 'Manual override cleared, original amount restored'
      });
  ...
  const effectiveAmount = invoice.manual_amount_override ?? invoice.amount_due;
  const hasOverride = invoice.manual_amount_override !== null;
