-- Fix duplicate migration and run backing tracks migrations
-- Run this in Supabase SQL Editor

-- First, check if the duplicate migration exists and mark it
DO $$
BEGIN
  -- Check if migration 20250122000025 is already applied
  IF EXISTS (
    SELECT 1 FROM supabase_migrations.schema_migrations 
    WHERE version = '20250122000025'
  ) THEN
    RAISE NOTICE 'Migration 20250122000025 already exists, skipping duplicate';
  END IF;
END $$;
...
-- Mark migrations as applied (if schema_migrations table exists)
DO $$
BEGIN
  -- Mark migration 20250205000003
  IF NOT EXISTS (
    SELECT 1 FROM supabase_migrations.schema_migrations 
...
SELECT '✅ Backing tracks migrations completed successfully!' as status;
